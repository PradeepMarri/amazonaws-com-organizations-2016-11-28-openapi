...
func DescribeorganizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		url := fmt.Sprintf("%s/#X-Amz-Target=AWSOrganizationsV20161128.DescribeOrganization", cfg.BaseURL)
		...
		var result models.DescribeOrganizationResponse
...
func CreateDescribeorganizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AWSOrganizationsV20161128_DescribeOrganization",
	...
	return models.Tool{
		Definition: tool,
		Handler:    DescribeorganizationHandler(cfg),
	}
}
